...
class BaseDeDatos {
  constructor() {
    ...
    this.agregarRegistro(8, "Arroz integral", 100, "Alimentos", "arroz.jpg");
    this.agregarRegistro(2, "Fideos", 50, "Alimentos", "fideos.jpg");
    this.agregarRegistro(3, "Alfajor", 25, "Alimentos", "alfajor.jpg");
    this.agregarRegistro(4, "Pan", 25, "Alimentos", "pan.jpg");
    this.agregarRegistro(6, "Atún", 200, "Alimentos", "atun.jpg");
  }
  ...
  // Registro por nombre asincrónico: simulamos un delay con setTimeout y lo manejamos
  // con una promesa. Si encontramos algo, retornamos el array con resolve, si no encontramos
  // nada, devolvemos un mensaje de error.
  registrosPorNombre(palabra) {
    // El mismo filter que el algoritmo anterior, busca productos que tengan un nombre
    // similar a la palabra que introducimos en el input
    const productosEncontrados = this.productos.filter((producto) =>
      producto.nombre.toLowerCase().includes(palabra)
    );
    // Pero esta vez lo retornamos en una promesa según lo que encuentre
    const promesa = new Promise((resolve, reject) => {
      // El setTimeout es una función asincrónica que se va a ejecutar
      // 4 segundos después de inciar la instrucción
      setTimeout(function () {
        if (productosEncontrados.length > 0) {
          // Si encuentra algo, definimos la promesa como exitosa, y le pasamos por
          // el parámetro/función resolve el array de productos encontrados
          // para poder recibirlo luego en el .then();
          resolve(productosEncontrados);
        } else {
          // Si no encuentra nada, le decimos a la promesa que hubo un error
          // usando el parámetro/función "reject", y lo podemos manejar luego en el .catch();
          reject(`No se encontraron productos en la búsqueda con la palabra ${palabra}.`);
        }
      }, 4000);
    });
    return promesa;
...
formBuscar.addEventListener("submit", (event) => {
  ...
  // Avisamos al usuario que estamos buscando los productos, pero todavía
  // no tenemos el resultado
  divProductos.innerHTML = "<p>Buscando productos...</p>";
  // Pedimos a nuestra "base de datos" que nos traiga todos los registros que coincida
  // con la palabra que le pusimos en el input
  const promesa = bd.registrosPorNombre(palabra);
  // Nos devuelve la promesa, así que trabajamos con ella
  promesa
    // En el método .then recibimos lo que le pasamos por resolve en la promesa, en este
    // caso el array con el resultado de la búsqueda
    .then((result) => {
      cargarProductos(result);
    })
    // En el método .catch recibimos lo que le pasamos por reject en la promesa, en este
    // caso un simple mensaje de error, que usamos Sweet Alert para mostrarlo
    .catch((error) => {
      cargarProductos(bd.traerRegistros());
      Swal.fire({
        title: "¡Uh!",
